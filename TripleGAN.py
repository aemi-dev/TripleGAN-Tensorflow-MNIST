...
class TripleGAN(object) :
    def __init__(self, sess, epoch, batch_size, unlabel_batch_size, z_dim, dataset_name, n, gan_lr, cla_lr, checkpoint_dir, result_dir, log_dir):
        ...
        if self.dataset_name == 'mnist' :
            ...
            self.data_X, self.data_y, self.unlabelled_X, self.unlabelled_y, self.test_X, self.test_y = mnist.prepare_data(n) # trainX, trainY, testX, testY
    ...
    def discriminator(self, x, y_, scope='discriminator', is_training=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse) :
            ...
            x = lrelu(linear(x, unit=1000, layer_name=scope+'_linear1'))
            x = weight_norm( x, 1000 )
            x = conv_concat(x,y)

            x = gaussian_noise_layer(x)
            x = lrelu(linear(x, unit=500, layer_name=scope+'_linear2'))
            x = weight_norm( x, 500 )
            x = conv_concat(x,y)

            x = gaussian_noise_layer(x)
            x = lrelu(linear(x, unit=250, layer_name=scope+'_linear3'))
            x = weight_norm( x, 250 )
            x = conv_concat(x,y)

            x = gaussian_noise_layer(x)
            x = lrelu(linear(x, unit=250, layer_name=scope+'_linear4'))
            x = weight_norm( x, 250 )
            x = conv_concat(x,y)

            x = gaussian_noise_layer(x)
            x = lrelu(linear(x, unit=250, layer_name=scope+'_linear5'))
            x = weight_norm( x, 250 )
            x = conv_concat(x,y)

            x = gaussian_noise_layer(x)
            x_logit = linear(x, unit=1, layer_name=scope+'_linear6')
            x_logit = weight_norm( x, 1 )
            out = sigmoid(x_logit)
    ...
    def generator(self, z, y, scope='generator', is_training=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse) :
            ...
            x = softplus(linear(x, unit=500, layer_name=scope+'_linear7'))
            x = batch_norm(x, is_training=is_training, scope=scope+'_batch1')
            x = conv_concat(x,y)

            x = softplus(linear(x, unit=500, layer_name=scope+'_linear8'))
            x = batch_norm(x, is_training=is_training, scope=scope+'_batch2')
            x = conv_concat(x,y)

            x = sigmoid(linear(x, unit=784, layer_name=scope+'_linear9'))

            return x
    def classifier(self, x, scope='classifier', is_training=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse) :

            x = lrelu(conv_layer(x, filter_size=32, kernel=[5,5], layer_name=scope+'_conv1'))
            x = lrelu(conv_layer(x, filter_size=64, kernel=[3,3], layer_name=scope+'_conv2'))
            ...
            x = lrelu(conv_layer(x, filter_size=64, kernel=[3,3], layer_name=scope+'_conv4'))
            ...
            x = flatten(x)
            x = linear(x, unit=10, layer_name=scope+'_linear10')
            return x
